...
論文の編集は Visual Studio Code (エディタ) + LaTeX Workshop (Visual Code内の拡張機能) を使うと便利である。Visual Studio Codeの設定ファイル (User Settings) に以下のコードを書いておけば、ファイルを編集し保存する度に自動的にコンパイル(文字列からPDFファイルに変換する作業)される。ちなみに、ptex2pdfを2度実行しているのは目次や図表や参考文献などのリンク(番号)を2度目で生成しているからである。
\begin{verbatim}
...
必須ではないが、Windowsユーザーの方は、PDFビューア(表示用ツール)にSumatraPDFを使うことを強くお勧めする。これは、コンパイルするたびに更新されるPDFファイルを自動的に更新して表示してくれるものである。Visual Studio Code内のブラウザでも構わない。
だが、Adobe Readerだけは絶対に使ってはならない。Adobe ReaderはPDFファイルを読み取り専用にするので、コンパイルが出来なくなる。
印刷するときのみ、Adobe Readerを使うようにしよう。

Visual Studio Code と SumatraPDFの組み合わせでは、SumatraPDFのオプションの逆順検索コマンドラインの設定で、\verb|"C:\Program Files\Microsoft VS Code\code.exe" -g "%f:%l"|などと書いておくと、PDFファイルの文章をクリックするとテキストファイルの元の文章の場所に飛ぶことができる。なお、\verb|code.exe|のパスは上記の限りではないので注意されたし。
...
吉本 雅浩 (岐阜大学 教育学部) yoshimoto@onsanai.com
...
\section{よくある質問(FAQ)}
\subsection{コンパイルが通らなくなってしまった}
経験上、99\%は文法ミスである。\verb|\end|や\verb|\begin|を忘れていないかをもう一度確認してみよう。
コンパイルが通らなくなった場所を特定するには、コンパイル時に設定した\verb|"-interaction=nonstopmode",|を削除してみるのも一つの手である。
この設定を外すと、エラーの箇所で停止してくれる。例えば、次のようなエラーが出たときは、\verb|\end{table}|を書き忘れたときであることが推定できる。
...
\subsection{内容は間違っていないのにコンパイルに失敗する}
PDFファイルをAdobe readerなどで開いてはいないだろうか。Adobe readerは、PDFファイルの書き込み権限を握るため、コンパイルでPDFファイルを上書きなくなる。コンパイルの最後でPDFファイル生成で失敗するのは、Adobe readerが原因であることが多い。
